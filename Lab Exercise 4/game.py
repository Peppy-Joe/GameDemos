...
def exit_leads_to(exits, direction):
    ...
    if direction in exits.keys():
        if exits[direction] is "Office":
            return "the general office"
        elif exits[direction] is "Admins":
            return "MJ and Simon's room"
        elif exits[direction] is "Tutor":
            return "your personal tutor's office"
        else:
...
def print_menu(exits):
    ...
    y = int(len(exits))
    while x != y:
        print_menu_line(list(exits)[x], exits[list(exits)[x]])
        x=x+1
    # COMPLETE THIS PART:
...
def menu(exits):
    ...
    while checking:
        ...
        check = bool(is_valid_exit(exits, user_input))
        if check:
            checking = False
        else:
            print("Invalid input, corret yourself")
...
def move(exits, direction):
    ...
    if direction == "north":
        if "north" in exits.keys():
            return rooms[exits.get("north")]
    elif direction == "south":
        if "south" in exits.keys():
            return rooms[exits.get("south")]
    elif direction == "east":
        if "east" in exits.keys():
            return rooms[exits.get("east")]
    elif direction == "west":
        if "west" in exits.keys():
            return rooms[exits.get("west")]
